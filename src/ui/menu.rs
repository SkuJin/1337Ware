...
use crate::utils::{config::{CONFIG, CONFIG_DIR, CONFIGS, Config, delete_config, ProgramConfig, DEFAULT_CONFIG, CONFIG_EXTENSION}, messagebox::{MessageBoxStyle, create_messagebox}};
...
pub fn render_menu(ui: &mut Ui) {
    ...
    ui.window("Skeet.cc")
        ...
        .build(|| {
            ...
            ui.separator();
            ui.text(format!("Hide/Show: {:?}", ProgramConfig::Keys::ToggleKey));

            ui.same_line();
            ui.text(format!("| Unload {:?}", ProgramConfig::Keys::ExitKey));
        });
});
}